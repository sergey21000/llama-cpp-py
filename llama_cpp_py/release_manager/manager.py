...
class LlamaReleaseManager(GithubReleaseManager):
    ...
    def __init__(
        ...
        exclude_patterns: list[str] | None = ['vulkan', 'cudart', 'rocm', 'openvino', 'sycl'],
        priority_patterns: list[str] | None = ['ubuntu-x64', 'win-cpu-x64', 'win-cuda'],
    ):
        ...
        if tag == 'latest':
            tag = os.getenv('LLAMACPP_RELEASE_TAG') or tag
        if not releases_dir:
            releases_dir = os.getenv('LLAMACPP_RELEASES_DIR')
            if not releases_dir:
                releases_dir = Path(
    ...
    def validate_release_dir(self, release_dir: Path) -> None:
        ...
        if not release_dir.exists():
            debug_logger.debug(f'Path {release_dir} not exists')
            return False
